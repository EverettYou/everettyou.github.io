...
If an LLM were simply making random, independent mistakes at each step (like rolling a die), its accuracy would drop off slowly and smoothly as the task got longer. But that's not what we found. Instead, we observed a phenomenon we call the **Accuracy Cliff**.
