# Rule: Teaching philosophy and course strategy

**Maintenance phase:** The note set is largely complete. Prefer incremental edits; see **`maintenance-scope.md`**.

Conventions overlap with PHYS 2C at `../../PHYS2C/notes_src/_design/design.md`. **PHYS130B rules win** when they conflict (notably: **never use `---` horizontal rules** in 130B).

## The problem (2026)

LLMs solve undergraduate physics homework at or above student level. Traditional teaching focused on knowledge transmission and problem-solving drill is no longer sufficient. We must rethink what physics education is for.
...
**3. In-class interaction (课堂互动):** Students bring AI to class. Two mechanisms:
   - **Poll boxes** (`dropdown poll`): quick multiple-choice conceptual checks for live voting. Professor displays the question, students vote, then the class discusses. At most 2 per lecture. See `content-style.md` § Poll Design.
   - **Discussion boxes** (`dropdown tip`): broad, high-level questions for TA-led discussion sessions. Not used during lecture — they serve as prompts for extended debate in sections.
