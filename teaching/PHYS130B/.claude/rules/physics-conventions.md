...
- **Gauge / rephasing function symbol — `\alpha` only.** Throughout Chapter 4, every gauge or rephasing function uses the symbol `\alpha`, regardless of context: EM gauge transformation (above); Berry-phase parameter-space rephasing `\vert n(\boldsymbol R)\rangle\to\mathrm{e}^{\mathrm{i}\alpha(\boldsymbol R)}\vert n(\boldsymbol R)\rangle` with `\boldsymbol A_n\to\boldsymbol A_n-\nabla_{\boldsymbol R}\alpha`; Berry connection in time `\phi_n\to\phi_n-\dot\alpha` under `\vert n(t)\rangle\to\mathrm{e}^{\mathrm{i}\alpha(t)}\vert n(t)\rangle`; Wu-Yang / Dirac-monopole transition function `\boldsymbol A^{(N)}-\boldsymbol A^{(S)} = \nabla\alpha` with overlap rephasing `\psi^{(N)} = \mathrm{e}^{\mathrm{i}q\alpha/\hbar}\psi^{(S)}`. Do **not** introduce `\chi`, `\Lambda`, or any other symbol for these roles. (`\chi` is reserved for unrelated uses such as a generic ket name in §1.3.2; in Ch 4 it must not appear.) `validate_project.py --scope ch4` flags any Ch 4 use of `\chi`.
...
- **Exception — spacetime-unified 4-vectors and tensors.** Lorentz-covariant presentations that use 4-vector or tensor notation (e.g. $A^\mu = (\phi, \boldsymbol{A})$, $\partial_\mu = (\partial_t, \nabla)$, $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu$, the Lorenz gauge $\partial_\mu A^\mu = 0$) may set $c = 1$ for compact notation. **State the convention explicitly** at the start of any block (admonition, homework problem, derivation) where it is used. Outside such 4-vector / tensor blocks, SI units (with explicit $c$, $\epsilon_0$, $\mu_0$) remain mandatory — in particular, the surrounding 3-vector statements $\boldsymbol{E} = -\nabla\phi - \partial_t\boldsymbol{A}$, $\boldsymbol{B} = \nabla\times\boldsymbol{A}$, and the minimal-coupling and Maxwell forms above must remain in SI.
- **Exception — flux-defined magnetic charge (Dirac-monopole convention).** Presentations of magnetic-monopole physics in the §4.4 spin / monopole sequence (notebooks `4-4-1-classical-spin`, `4-4-2-dirac-monopole`, `4-4-3-monopole-harmonics`) may use the **flux-defined** convention in which the magnetic charge $g$ is identified directly with the total magnetic flux of a point monopole, absorbing the $\mu_0$ factor. In this convention: point-monopole field $\boldsymbol{B} = g\,\hat{\boldsymbol{r}}/(4\pi r^{2})$, modified Gauss law $\nabla\cdot\boldsymbol{B} = \rho_m$, total flux $\oint \boldsymbol{B}\cdot\mathrm{d}\boldsymbol{S} = g$, and Dirac quantization $qg = 2\pi n\hbar$. **State the convention explicitly** at the start of the lecture section (or admonition / derivation / homework problem block) where it is used, modeled on the $c=1$ precedent above. Outside such declared blocks — including the rest of Chapter 4's electrostatic / magnetostatic statements, Maxwell equations, and ambient EM material — the SI forms above ($\boldsymbol{B} = \mu_0 g\,\hat{\boldsymbol{r}}/(4\pi r^{2})$, $\nabla\cdot\boldsymbol{B} = \mu_0\rho_m$, total flux $\mu_0 g$, and Dirac quantization expressed in those SI variables) remain mandatory. `validate_project.py --scope ch4` already exempts the three §4.4 notebooks above from the SI-point-source check; do **not** extend the exception to other Chapter 4 notebooks.
